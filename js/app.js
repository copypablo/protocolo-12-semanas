...
function atualizarGaleria() {
    ...
    if (fotosSalvas.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 40px; color: var(--gray-color);">
                <p style="font-size: 18px; margin-bottom: 10px;">Nenhuma foto adicionada ainda.</p>
                <p>Registre seu progresso diário para visualizar aqui!</p>
            </div>
        `;
        return;
    }
    ...
    fotosSalvas.forEach((foto, index) => {
        ...
        const img = document.createElement('img');
        img.src = foto.foto;
        img.alt = `Foto do dia ${foto.dia}`;
        
        const date = document.createElement('div');
        date.classList.add('date');
        date.textContent = `Dia ${foto.dia} - ${foto.data}`;
...
function filtrarFotos(filtro) {
    ...
    // Filtrar fotos
    let fotosFiltradas = [...fotosSalvas];
    
    if (filtro === 'recent') {
        fotosFiltradas = fotosFiltradas.slice(-10); // Últimas 10 fotos
    } else if (filtro === 'oldest') {
        fotosFiltradas = fotosFiltradas.slice(0, 10); // Primeiras 10 fotos
    }
    
    // Adicionar fotos filtradas à galeria
...
function mostrarFoto(foto) {
    // Criar modal para mostrar a foto
    ...
    title.textContent = `Dia ${foto.dia} - ${foto.data}`;
    title.style.marginBottom = '10px';
    title.style.color = 'var(--primary-color)';
    title.style.fontSize = '18px';
    
    const obs = document.createElement('p');
    obs.textContent = foto.observacoes || 'Sem observações';
    obs.style.lineHeight = '1.6';
...
function exportarDadosLocal() {
    ...
    // Converter para JSON
    const jsonString = JSON.stringify(dadosExportacao);
    
    // Criar link para download
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(jsonString);
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", "desafio_90_dias_backup.json");
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
    
    alert('Backup salvo com sucesso! O arquivo foi baixado para o seu computador.');
}

function exportarParaPlanilha() {
    // Abrir a planilha em uma nova aba
    window.open('https://docs.google.com/spreadsheets/d/1s4clTbTO_5KezVfKwE4ze9Ov6lb55XAM-px_ax2r9g4/edit', '_blank');
    
    // Mostrar instruções
    alert('A planilha foi aberta em uma nova aba. Para exportar seus dados:\n\n1. Copie os dados que deseja exportar\n2. Cole na planilha do Google Sheets\n\nPara exportar fotos, você pode arrastar e soltar as imagens diretamente na planilha.');
}
